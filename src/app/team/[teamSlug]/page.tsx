...
import { footballTeams, supportedSports } from '@/lib/mockData'; // Renamed teams to footballTeams
...
// Corrected imports for football-specific functions
import { 
...
const SEASON_FOR_FOOTBALL_MATCHES = 2023; // Renamed from SEASON_FOR_MATCHES
...
export default function OldTeamProfilePage() { // Renamed component to avoid conflict if any
  const params = useParams();
  const teamSlug = params.teamSlug as string;
  const sportSlug = 'football'; // This page is hardcoded for football as it's the old one
  const router = useRouter();
  const { currentUser } = useAuth();
  const { toast } = useToast();

  const [teamDetails, setTeamDetails] = useState<TeamApp | null>(null);
  const [mockTeamData, setMockTeamData] = useState<TeamApp | null>(null); // Changed Team to TeamApp
  ...
  // Define currentSport for this football-specific page
  ...
  const fetchTeamPageData = useCallback(async (apiTeamId: number, teamNameFromMock: string) => {
    if (!currentSport) {
        toast({ variant: 'destructive', title: 'Configuration Error', description: 'Football sport definition not found.' });
        setIsLoadingData(false);
        return;
    }
    const footballApiBaseUrl = currentSport.apiBaseUrl;
    ...
    try {
      // Use corrected function names and pass footballApiBaseUrl
      const detailsPromise = getFootballTeamDetails(apiTeamId, footballApiBaseUrl);
      const pastMatchesPromise = getFootballMatchesForTeam(apiTeamId, SEASON_FOR_FOOTBALL_MATCHES, { status: 'FT' }, footballApiBaseUrl);
      const coachPromise = getFootballCoachForTeam(apiTeamId, footballApiBaseUrl);
      const squadPromise = getFootballSquadForTeam(apiTeamId, footballApiBaseUrl);
  ...
  }, [toast, currentSport]); // Added currentSport to dependencies

  useEffect(() => {
    if (teamSlug) {
      const foundMockTeam = footballTeams.find((t) => t.slug === teamSlug);
      setMockTeamData(foundMockTeam || null); // Use footballTeams
  ...
  const displayTeamLogo = teamDetails?.logoUrl || mockTeamData?.logoImageUrl; // Use logoImageUrl from TeamApp
  ...
  if (!mockTeamData && !isLoadingData && !currentSport) { // Added check for currentSport
    notFound();
  }
  
  // Fallback if currentSport is not defined (should not happen if slug is 'football')
  if (!currentSport) {
    return (
         <div className="flex flex-col min-h-screen bg-background">
            <Header />
            <main className="flex-grow container mx-auto px-4 py-8 flex justify-center items-center">
                <p>Sport configuration error.</p>
            </main>
            <Footer />
        </div>
    );
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        <div className="mb-6">
            {/* Link back to the new football teams page structure */}
            <Button variant="outline" asChild>
                <Link href={`/sports/football/teams`}>
                    <ChevronLeft size={18} className="mr-2" />
                    Back to Football Teams
                </Link>
            </Button>
        </div>
        ...
        <div className="mt-12 text-center">
             {/* Link back to the main sports selection page */}
            <Link href="/">
