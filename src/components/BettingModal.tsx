...
interface BettingModalProps {
  isOpen: boolean;
  onClose: () => void;
  eventData: MatchApp | ManagedEventApp; // Can be an API match or a custom managed event
...
export function BettingModal({ isOpen, onClose, eventData, eventSource, teamToBetOn, currentUser, sportSlug }: BettingModalProps) {
  ...
  const homeTeamName = 'homeTeam' in eventData ? eventData.homeTeam.name : eventData.homeTeamName;
  const awayTeamName = 'awayTeam' in eventData ? eventData.awayTeam.name : eventData.awayTeamName;
  const eventTitle = eventSource === 'api' ? 'API Match' : 'Custom Event';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Place Bet on {sportSlug} {eventTitle}</DialogTitle>
          <DialogDescription>
